...
5. **TIDAL normalization drops ambiguous entries**: the historical all-or-nothing `canonicalize_many()` helper was removed after it had no source callers. The live TIDAL path uses `collect_clear_genres()` and silently drops ambiguous entries.
