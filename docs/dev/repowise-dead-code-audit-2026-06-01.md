...
| `frontend/src/lib/components/TrendingCard.svelte` | Current tree has no file at this path. Exact `rg` finds only audit docs and the `TrendingShelf` contract test negative assertion; live chart routes use `TrendingShelf.svelte` with `ChartMural`. |
...
| `noor-server/src/db/queries.rs::get_cached_spotify_playcounts_for_isrcs` | Exact `rg` found no live route, service, frontend, script, or doc callers; rust-analyzer references found only the definition and two dedicated unit tests. Removed the test-only helper and tests. `cargo test -p noor-server library_search_favorites_lead_over_play_count` passes, and `cargo check -p noor-server` now reports 18 warnings after an escalated rerun because the sandboxed run hit `target` metadata access denial. |
| `noor-server/src/db/queries.rs::{get_embedding_model_by_key, get_embedding_models_by_family, rollback_to_ready_embedding_model, get_active_embedding_model}` | Exact `rg` and rust-analyzer found no live route, service, frontend, or script callers after removing the dedicated helper test. The surviving model insertion test now verifies the live `get_selected_discovery_embedding_model` path instead. `cargo test -p noor-server embedding_model`, `cargo test -p noor-server selected_discovery_model_lookup_uses_configured_engine_family`, `cargo check -p noor-server`, and `cargo fmt --all -- --check` pass; `cargo check` now reports 14 warnings with five remaining DB-query warning groups. |
| `noor-server/src/db/queries.rs::{ExternalPruneResult, replace_external_candidate_audio_feature, replace_external_candidate_embedding, prune_expired_external_candidates}` | Exact `rg` and rust-analyzer found no live route, service, frontend, or script callers. Removed the unwired prune API and the test-only feature and embedding replacement helpers, while keeping live external-candidate merge, neighbor, training, automix, and discovery route paths intact. The merge sidecar test now seeds feature and embedding rows directly with parameterized SQL. `cargo test -p noor-server external_candidate_merge_moves_sidecar_rows_before_deleting_loser`, `cargo test -p noor-server external_candidates_for_training_skip_expired_and_resolved_rows`, `cargo check -p noor-server`, and `cargo fmt --all -- --check` pass; `cargo check` now reports 10 warnings with one remaining DB-query warning group. |
| `noor-server/src/playback/queue.rs::{apply_shuffle, reorder_tracks}` | Exact `rg` and rust-analyzer found only queue-module tests for these older nondeterministic wrappers. Production shuffle routes already use `apply_shuffle_with_seed` and `reorder_tracks_with_seed` for persisted debug seed metadata. Removed the wrappers, moved tests to the seeded helpers, and updated stale shuffle docs/comments. `cargo test -p noor-server apply_shuffle_preserves_pending_artist_title`, `cargo test -p noor-server reorder_`, `cargo test -p noor-server playback_shuffle_returns_debug_and_persists_seed`, `cargo check -p noor-server`, and `cargo fmt --all -- --check` pass; `cargo check` now reports seven playback/output warning groups. |
...
| Player store exports in `frontend/src/lib/stores/player.ts` | Exact `rg` found the Repowise player-store candidates imported by live layout controls, automix settings, queue controls, playlist routes, Spotify routes, remote UI, charts, genre, search, home shelves, or player-store tests. Playback and queue behavior stayed unchanged. |
...
| DiscoverSpace internal exports in `discover_space_adapter.ts`, `discover_space_physics.ts`, and `discover_space_store.ts` | Exact `rg` found `deterministicInitialPosition`, `normalizeReason`, `normalizeReasonTags`, `adaptNode`, `adaptEdge`, `PhysicsConfig`, `normalizeBlendSeeds`, and `blendSeedFromNode` used only inside their defining modules after the colocated store test moved blend dedupe coverage through `addBlendSeed`. Removed the public export surface while preserving the live `adaptResponse`, physics functions, renderer functions, websocket refresh path, and route-facing store actions. |
| `frontend/src/lib/components/DiscoverSpace/discover_space_store.ts::clearRadioRoute` | Exact `rg` found only the store definition plus historical Repowise reports. The live route generation path still imports `setRadioRoute`, and `DiscoverSpace.svelte` still renders `radioRoute` state. |
...
| `noor-server/src/playback/{dj_engine.rs, player.rs, runtime/mod.rs, wasapi_exclusive.rs}` cargo-check dead-code warnings | Playback, queue, WASAPI, and output-switching boundaries are manual-review only by repo rule. `cargo check -p noor-server` on 2026-06-04 still reports seven playback/output warning groups around DJ lookahead, transition planning, prepared mixer state, shared fallback output, and exclusive output after removing the obsolete queue shuffle wrappers. |
...
| `noor-server/src/db/queries.rs::promote_temporary_audio_dj_profile` | Exact refs initially showed only query tests, a runtime test, and DJ-engine plan notes. The plan says pending queue profiles should move to the stable TIDAL key after resolution. | Wired through `playback::pending::promote` after a queue row wins atomic promotion and has a resolved TIDAL id. `cargo test -p noor-server playback::pending::tests::promote`, `cargo test -p noor-server pending_profile_promotes_after_tidal_resolution`, `cargo test -p noor-server promote_temporary_audio_dj_profile_copies_to_stable_key`, `cargo check -p noor-server`, and `cargo fmt --all -- --check` pass; `cargo check` now reports no DB-query warnings. |
...
| --- | --- | --- |
| Backend dead-code cleanup | `noor-server/src/genre/*`, `noor-server/src/services/audio_analysis/*`, `noor-server/src/services/charts/kworb_matrix.rs`, `noor-server/src/services/discovery.rs`, `noor-server/src/services/spotify_public/client.rs`, `noor-server/src/smart/*`, plus the genre and audit docs. | Playback/runtime/WASAPI warnings, TIDAL auth, and radio config suppressed fields until their manual-review slices are done. |
| Frontend isolated dead exports | Resolved in the current tree: cache exports, audio-analysis store wrappers, route helper removals, remote sleep timer export, `resolveGenreIds` export cleanup, and `TrendingCard.svelte`. | Mixed artwork-surface files and unrelated app-shell or route layout changes remain excluded. |
| Duplicate helper cleanup | Resolved in the current tree for `demo-random`, `tidalHomeItemToPlayable`, `initials`, and `formatTotalDuration`. | Artwork fallback changes remain excluded from helper-only cleanup. |
...
| --- | --- | --- |
| Frontend isolated dead exports | Resolved in the current tree. | Current exact `rg` finds only historical docs for `peekCachedValue`, `scheduleTargetedRefetch`, `startAnalysis`, `stopAnalysis`, `routePathsForIds`, `sleepTimerActive`, and `appRoutePath`; `resolveGenreIds` is now private and called only by `buildAudioParams`; `TrendingCard.svelte` is absent, with only the `TrendingShelf` negative contract assertion remaining in source. |
| Fixture demo-random consolidation | Resolved in the current tree. | `demo-kpis.ts`, `demo-ridgeline.ts`, `demo-sonic-field.ts`, and `demo-tempo.ts` import `mulberry32`, `gaussian`, and `dateNDaysAgo` from `frontend/src/lib/fixtures/demo-random.ts`. Exact `rg` finds those function definitions only in the shared helper file. |
| Duplicate helper cleanup in UI callers | Resolved in the current tree. | Exact `rg` finds `tidalHomeItemToPlayable`, `initials`, and `formatTotalDuration` definitions only in shared utility modules, with callers using those shared helpers. Artwork fallback changes remain a separate visual bucket. |
| Artwork/UI fallback bucket | Resolved in the current tree for contract tests, typecheck, lint, and production build. Browser smoke was not rerun in this slice; dynamic local and TIDAL detail routes still need backend/dev-server validation before calling the UI bucket fully verified. | Changed surfaces include app shell artwork, player bar, album and artist routes, search, library, remote routes, automix, duplicates, playlists, TrackRow/TidalTrackRow, video cards, DiscoverSpace, and home shelves. Current exact `rg` found no arbitrary TIDAL artwork sizes such as 256 or 512 in `frontend/src`; observed sizes are from the allowed set. `pnpm test -- src/lib/utils/artwork.test.ts src/lib/utils/tidal_artwork_surface_contract.test.ts src/lib/components/album_artwork_contract.test.ts src/lib/components/track_row_contract.test.ts src/lib/components/quiet_mode_layout_contract.test.ts src/lib/components/DiscoverSpace/discover_space_artwork_contract.test.ts src/lib/components/home/tidal_home_artwork_contract.test.ts src/lib/components/remote/remote_artwork_contract.test.ts src/lib/components/remote/remote_queue_artwork_contract.test.ts src/lib/components/remote/remote_mini_search_contract.test.ts src/lib/components/search/tidal_discover_artwork_contract.test.ts src/lib/components/video/video_card_artwork_contract.test.ts src/routes/albums/album_layout_contract.test.ts src/routes/artists/artist_layout_contract.test.ts src/routes/automix/automix_artwork_contract.test.ts src/routes/library/library_detail_artwork_contract.test.ts src/routes/library/library_home_contract.test.ts src/routes/library/library_layout_contract.test.ts src/routes/playlists/playlist_artwork_contract.test.ts src/routes/remote/remote_detail_artwork_contract.test.ts src/routes/remote/remote_library_artwork_contract.test.ts src/routes/search/search_layout_contract.test.ts` passed with 22 files and 68 tests. `pnpm lint` passed. `pnpm check` passed with 0 errors and 0 warnings after rerunning outside the sandbox because the sandboxed run hit `.svelte-kit` EPERM. `pnpm run build` passed after rerunning outside the sandbox because the sandboxed run hit Vite `spawn EPERM`. Previous browser smoke via `pnpm preview --host 127.0.0.1 --port 4173` and system Edge passed with 0 broken images, 0 page errors, and 0 console errors on `/`, `/library`, `/search`, `/remote`, `/remote/library`, `/remote/albums/1`, `/remote/artists/1`, `/automix`, `/duplicates`, `/playlists`, and `/charts`. Static preview returned 404 for `/albums/1`, `/artists/1`, `/tidal/albums/1`, and `/tidal/artists/1`; those had no broken images or page errors, but need backend/dev-server route validation for real detail states. |
| DiscoverSpace export surface | Resolved in the current tree for locally validated module-internal exports. | Exact `rg` found `deterministicInitialPosition`, `normalizeReason`, `normalizeReasonTags`, `adaptNode`, `adaptEdge`, `PhysicsConfig`, `normalizeBlendSeeds`, and `blendSeedFromNode` used only inside their defining modules after the test-only `normalizeBlendSeeds` import moved to public `addBlendSeed` coverage, so only their `export` modifiers were removed. Exact `rg` found `clearRadioRoute` only as a definition plus historical Repowise reports, so it was removed. Exact `rg` also found `addBlendDiscoveries`, `playBlendDiscoveries`, `makeBlendRadio`, `unlockSeed`, `addVisitedRegion`, `setRadioRoute`, `setRefreshProgress`, and `handleDiscoverySpaceRefreshed` imported by the route page, Svelte components, or websocket handling, so those remain public. |
