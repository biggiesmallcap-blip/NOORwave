...
| --- | --- |
| Remaining DiscoverSpace route-facing store exports | Store actions are imported by Svelte components, the route page, websocket handling, or contract tests. Exercise the DiscoverSpace surface before deleting anything beyond locally validated module-internal exports and the removed definition-only route clear helper. |
