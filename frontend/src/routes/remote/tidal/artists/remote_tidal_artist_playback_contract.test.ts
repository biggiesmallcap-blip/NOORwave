...
describe('remote TIDAL artist playback contract', () => {
	test('bulk and radio actions preserve discography track metadata', () => {
		...
		expect(source).toContain('playTidalTracksNow(p.top_tracks.map(artistTrackPlayable)');
		expect(source).toContain('shuffleTidalTracksNow(p.top_tracks.map(artistTrackPlayable)');
		expect(source).toContain('startTidalSongRadio(artistTrackPlayable(p.top_tracks[0]))');
