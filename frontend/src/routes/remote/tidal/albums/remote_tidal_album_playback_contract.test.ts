...
describe('remote TIDAL album playback contract', () => {
	test('play all reuses the loaded album track list', () => {
		...
		expect(source).toContain("onPlay={() => playTidalTracksNow(tracks.map(tidalDiscographyTrackToPlayable), header?.title ?? 'album')}");
		expect(source).toContain("onShuffle={() => shuffleTidalTracksNow(tracks.map(tidalDiscographyTrackToPlayable), header?.title ?? 'album')}");
